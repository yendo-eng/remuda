...
func TestVibeLaunchesInExistingWorkspace(t *testing.T) {
	...
	require.Contains(t, recorded.CommandRan, fmt.Sprintf("export BD_ACTOR='%s'", expectedSession))
	require.Contains(t, recorded.CommandRan, fmt.Sprintf("cd '%s'", workspace))
	require.Contains(t, recorded.CommandRan, "REMUDA_AGENT='custom'")
...
func TestVibeClaudeContainerComposesHermeticDockerCommand(t *testing.T) {
	...
	require.Contains(t, recorded.CommandRan, "REMUDA_AGENT='claude'")
	require.Contains(t, recorded.CommandRan, "REMUDA_MODEL='claude-sonnet-4'")
