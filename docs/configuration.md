...
- `REMUDA_REASONING_LEVEL` – default reasoning level for supported agents.
  For codex this uses `none|minimal|low|medium|high|xhigh`, with `max` or `ultra`
  available for GPT-5.6 and GPT-6 models that support each level; unknown values
  are passed through with a warning for Codex to validate. For claude this is
  forwarded to Claude Code as `--effort`.
