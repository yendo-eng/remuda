...
func resolvePromptFromStdin(in io.Reader, promptArg string) (string, bool, error) {
	...
	if in == nil {
		return "", true, pkgerrors.New("cannot read prompt from STDIN: stdin is nil")
	}

	b, err := io.ReadAll(in)
	if err != nil {
		return "", true, pkgerrors.Wrap(err, "read prompt from STDIN")
	}
	...
	if strings.TrimSpace(prompt) == "" {
		return "", true, pkgerrors.New("prompt from STDIN is empty")
	}
